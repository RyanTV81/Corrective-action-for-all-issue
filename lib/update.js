...
async function fetchWithLimit(url, asText) {
  const res = await fetch(url, {
    ...
    headers: { 'user-agent': `find-corrective-action/${APP_VERSION}` },
    redirect: 'follow'
  });
  if (!res.ok) throw new Error(`HTTP ${res.status} ${res.statusText} — ${url}`);
  const buf = Buffer.from(await res.arrayBuffer());
  if (buf.length > MAX_BYTES) throw new Error(`파일이 너무 큽니다(${buf.length} bytes): ${url}`);
...
function currentStatus() {
  ...
  return {
    ...
    counts: data.counts,
    manifestUrl: config.get().updateUrl || '',
    backups: listBackups().length
  };
}

async function check() {
  const url = (config.get().updateUrl || '').trim();
  if (!url) {
