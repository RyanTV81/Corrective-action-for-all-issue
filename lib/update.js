...
function currentStatus() {
  ...
  return {
    ...
    bundledVersion: bundled ? bundled.kbVersion : '',
    backups: listBackups().length
  };
}

/**
 * 원본 1: 프로그램과 함께 배포된 지식베이스 (deploy/kb-seed).
 *
 * 자동 배포(git)는 kb/ 를 건드리지 않는다 — 사용자가 쌓아온 지식베이스를 코드 배포가
 * 말없이 덮어쓰면 안 되기 때문이다. 대신 새 내용을 deploy/kb-seed 로 실어 보내고,
 * 관리자가 [지식베이스 업데이트]에서 확인하고 직접 적용하게 한다(백업·롤백은 동일).
 * 그래서 서버에 접속해 파일을 복사하지 않아도 화면에서 반영할 수 있다.
 *
 * learned.json(관리자가 [KB 등록]으로 추가한 불량)은 이 서버에만 있는 내용이므로 목록에서 제외한다.
 */
function bundledSource() {
  const dir = path.join(ROOT, 'deploy', 'kb-seed');
  let v;
  try {
    v = JSON.parse(fs.readFileSync(path.join(dir, 'version.json'), 'utf8'));
  } catch (e) {
    return null; // 배포본에 지식베이스가 없는 설치 형태
  }
  if (!v || !v.kbVersion) return null;

  const files = [];
  const add = (rel) => {
    const abs = path.join(dir, rel);
    if (!fs.existsSync(abs)) return;
    safeKbPath(rel); // kb/ 밖으로 나가는 경로는 거부 (온라인 경로와 같은 검증)
    files.push({ path: rel, abs, bytes: fs.statSync(abs).size });
  };

  add('processes.json');
  const defectDir = path.join(dir, 'defects');
  if (fs.existsSync(defectDir)) {
    for (const n of fs.readdirSync(defectDir).filter((x) => x.endsWith('.json') && x !== 'learned.json').sort()) {
      add('defects/' + n);
    }
  }
  ...
  return {
    kind: 'bundled',
    label: '프로그램에 포함된 지식베이스',
    kbVersion: v.kbVersion,
    releasedAt: v.updatedAt || null,
    notes: v.notes || '',
    files
  };
}

/** 원본 2: 인터넷 업데이트 서버(매니페스트) */
async function onlineSource(url) {
  ...
  if (manifest.minAppVersion && cmpVersion(APP_VERSION, manifest.minAppVersion) < 0) {
    throw new Error(`이 업데이트는 앱 ${manifest.minAppVersion} 이상이 필요합니다 (현재 ${APP_VERSION})`);
  }
...
/**
 * 두 원본 중 버전이 더 높은 쪽을 고른다. 같으면 내려받지 않아도 되는 배포본 쪽을 쓴다.
 * 인터넷 쪽이 실패해도 배포본이 있으면 그것으로 진행한다(오프라인 VM 대응).
 */
async function check() {
  ...
  if (url) {
    try {
      candidates.push(await onlineSource(url));
    } catch (e) {
      onlineError = e;
    }
  }
  ...
  if (!candidates.length) {
    ...
    const e = new Error('업데이트 원본이 없습니다. [설정]에서 매니페스트 URL을 등록하세요.');
    e.code = 'NO_MANIFEST_URL';
    throw e;
  }

  candidates.sort((a, b) => cmpVersion(b.kbVersion, a.kbVersion) || (a.kind === 'bundled' ? -1 : 1));
  const src = candidates[0];

  return {
    currentVersion: cur,
    latestVersion: src.kbVersion,
    updateAvailable: cmpVersion(src.kbVersion, cur) > 0,
    source: src.kind,
    sourceLabel: src.label,
    releasedAt: src.releasedAt,
    notes: src.notes,
    fileCount: src.files.length,
    files: src.files.map((f) => ({ path: f.path, bytes: f.bytes || null })),
    // 인터넷 확인은 실패했지만 배포본으로 진행하는 경우, 왜 그런지 화면에 알려준다
    onlineError: onlineError ? onlineError.message : '',
    manifest: src
  };
}
...
async function apply() {
  ...
  if (!info.updateAvailable) {
    // manifest 는 서버 내부 정보(파일 경로)라 화면으로 내보내지 않는다
    const { manifest, ...rest } = info;
    return { applied: false, reason: '이미 최신 버전입니다', ...rest };
  }

  /* 1) 전부 모아서 검증 (하나라도 실패하면 아무것도 쓰지 않음) */
  const src = info.manifest;
  const staged = [];
  for (const f of src.files) {
    const abs = safeKbPath(f.path);
    // 배포본은 같은 서버의 파일이라 내려받지 않고 그대로 읽는다
    const buf = src.kind === 'bundled' ? fs.readFileSync(f.abs) : await fetchWithLimit(f.url, false);
  ...
  try {
    ...
    kb.writeVersion({
      ...
      source: src.kind === 'bundled' ? '프로그램에 포함된 지식베이스 (deploy/kb-seed)' : config.get().updateManifestUrl,
      channel: src.kind === 'bundled' ? 'bundled' : 'online',
      notes: info.notes
    });
  } catch (e) {
