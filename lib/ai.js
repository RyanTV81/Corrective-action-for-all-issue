...
function client() {
  const c = config.get();
  const apiKeys = [c.apiKey, c.apiKey2].filter(Boolean);
  if (!apiKeys.length) {
...
function enabled() {
  const c = config.get();
  return Boolean(c.apiKey || c.apiKey2);
}
...
// 키가 2개 등록된 경우 호출마다 번갈아 사용해(라운드로빈) 각 키의 분당 요청 한도를 나눠 쓴다.
let rrCounter = 0;

async function callGemini({ apiKeys, keyIdx, model, body, timeoutMs = 150000, retryOn429 = true, _retried = false }) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 429 && retryOn429 && !_retried) {
      if (apiKeys.length > 1) {
        // 다른 키는 별도의 무료 한도(RPM)를 가지므로 대기 없이 바로 그 키로 전환해 재시도
        return callGemini({ apiKeys, keyIdx: idx + 1, model, body, timeoutMs, retryOn429, _retried: true });
      }
      const waitSec = Math.min(Math.max(parseRetryDelaySeconds(json, rawMsg) || 10, 3), 45);
      await sleep(waitSec * 1000);
      return callGemini({ apiKeys, keyIdx: idx, model, body, timeoutMs, retryOn429, _retried: true });
    }
