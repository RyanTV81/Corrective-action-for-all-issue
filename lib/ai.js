...
async function callGemini({ apiKey, model, body, timeoutMs = 150000, retryOn429 = true, _retried = false }) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 429 && retryOn429 && !_retried) {
      ...
      return callGemini({ apiKey, model, body, timeoutMs, retryOn429, _retried: true });
    }
...
async function callWithSearchTool({ apiKey, model, body, timeoutMs }) {
  try {
    return await callGemini({ apiKey, model, body: { ...body, tools: [{ google_search: {} }] }, timeoutMs, retryOn429: false });
  } catch (e) {
    if (e.code !== 'BAD_REQUEST') throw e;
    return await callGemini({ apiKey, model, body: { ...body, tools: [{ googleSearch: {} }] }, timeoutMs, retryOn429: false });
  }
}
