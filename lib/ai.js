...
async function callGemini({ apiKey, model, body, timeoutMs = 150000 }) {
  ...
  if (!res.ok) {
    const msg = (json && json.error && json.error.message) || `Gemini API 오류 (HTTP ${res.status})`;
...
/**
 * @param {object} opt {images:[{data,mediaType}], note, processId}
 */
...
한국어와 영어 기술용어를 모두 사용해 검색하라. 조사 결과를 근거와 함께 상세히 서술하라.`;
...
중복되는 내용을 채워 넣지 말고, 서로 다른 관점의 10가지를 제시하라. 반드시 JSON 스키마에 맞는 JSON만 출력하라.`;
