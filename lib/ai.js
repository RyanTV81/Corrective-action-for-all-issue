...
function client() {
  const c = config.get();
  const apiKeys = [c.apiKey, ...c.apiKeysExtra].filter(Boolean);
  if (!apiKeys.length) {
    const err = new Error('Gemini API 키가 설정되지 않았습니다. 대시보드 [설정]에서 등록하세요.');
    err.code = 'NO_API_KEY';
    throw err;
  }
  return { apiKeys, model: c.model, effort: c.effort };
}

function enabled() {
  const c = config.get();
  return Boolean(c.apiKey) || c.apiKeysExtra.length > 0;
}
...
// 키가 여러 개 등록된 경우 호출마다 번갈아 사용해(라운드로빈) 각 키의 분당 요청 한도를 나눠 쓴다.
let rrCounter = 0;

async function callGemini({ apiKeys, keyIdx, model, body, timeoutMs = 150000, retryOn429 = true, attempt = 0 }) {
  const idx = keyIdx === undefined ? rrCounter++ % apiKeys.length : ((keyIdx % apiKeys.length) + apiKeys.length) % apiKeys.length;
  ...
  if (!res.ok) {
    ...
    // 무료 한도의 짧은 버스트 제한(RPM)에 걸린 경우
    // (retryOn429=false 인 호출은 무료 DuckDuckGo 대체 경로가 바로 있으므로 여기서 재시도로 한도를 더 깎지 않고 즉시 넘어간다)
    if (res.status === 429 && retryOn429) {
      const triedAllKeys = attempt + 1 >= apiKeys.length;
      if (!triedAllKeys) {
        // 아직 시도하지 않은 다른 키가 남아있으면, 별개의 무료 한도(RPM)를 가지므로 대기 없이 바로 전환해 재시도
        return callGemini({ apiKeys, keyIdx: idx + 1, model, body, timeoutMs, retryOn429, attempt: attempt + 1 });
      }
      if (attempt === 0) {
        // 등록된 모든 키를 다 돌았는데도(키가 1개뿐이었던 경우 포함) 처음 겪는 상황이면, 한 번만 더 대기 후 마지막 재시도
        const waitSec = Math.min(Math.max(parseRetryDelaySeconds(json, rawMsg) || 10, 3), 45);
        await sleep(waitSec * 1000);
        return callGemini({ apiKeys, keyIdx: idx, model, body, timeoutMs, retryOn429, attempt: attempt + 1 });
      }
    }
 ...
 */
async function callWithSearchTool({ apiKeys, model, body, timeoutMs }) {
  try {
    return await callGemini({ apiKeys, model, body: { ...body, tools: [{ google_search: {} }] }, timeoutMs, retryOn429: false });
  } catch (e) {
    if (e.code !== 'BAD_REQUEST') throw e;
    return await callGemini({ apiKeys, model, body: { ...body, tools: [{ googleSearch: {} }] }, timeoutMs, retryOn429: false });
  }
}
...
/**
 * 확정된 교정 사례의 실제 사진을 few-shot 참고 이미지로 읽어들인다 (최대 2건 · 각 1장).
 * 텍스트 설명만으로는 촬영 각도·조명 문제를 모델이 체감하기 어려워, 실제 참고 사진을 함께 준다.
 */
function learningImageParts(examples) {
  const parts = [];
  const withImages = (examples || []).filter((e) => e && e.defectName && (e.imageUrls || []).length).slice(0, 2);
  if (!withImages.length) return parts;

  parts.push({ text: '\n\n[참고 사례 사진] 아래는 현장에서 실제로 확인·정정된 과거 사례 사진이다. 참고만 하고, 최종 판정은 이 뒤에 나오는 "이번 분석 대상 사진" 기준으로 하라.' });
  for (const e of withImages) {
    const url = e.imageUrls[0];
    try {
      const abs = path.join(UPLOAD_DIR, path.basename(url));
      if (!abs.startsWith(UPLOAD_DIR) || !fs.existsSync(abs)) continue;
      const ext = path.extname(abs).toLowerCase();
      const mediaType = MIME_BY_EXT[ext];
      if (!mediaType) continue;
      parts.push({ text: `사례: ${e.defectName}` });
      parts.push({ inlineData: { mimeType: mediaType, data: fs.readFileSync(abs).toString('base64') } });
    } catch (e2) {
      /* 원본 파일이 삭제 등으로 없으면 조용히 건너뜀 */
    }
  }
  if (parts.length === 1) return []; // 텍스트 라벨만 남고 이미지가 하나도 안 붙었으면 그냥 생략
  parts.push({ text: '[이번 분석 대상 사진]' });
  return parts;
}
...
async function analyzeImage(opt) {
  const { apiKeys, model, effort } = client();
  const data = kb.load();
  ...
  const hintedProc = opt.processId ? kb.getProcess(opt.processId) : null;
  const hintedProcess = hintedProc
    ? `\n\n작업자가 지정한 공정: ${hintedProc.name} (${hintedProc.id}). 사진이 명백히 다른 공정이 아니면 이 공정으로 판정하라.`
    : '';
...
confidence 는 0~1 사이 값이다. 반드시 JSON 스키마에 맞는 JSON만 출력하라.${langDirective(opt.lang)}`;
  ...
  if (opt.images && opt.images.length) {
    parts.push(...learningImageParts(opt.learningExamples));
  }
  ...
  const resp = await callGemini({ apiKeys, model, body });
 ...
 */
async function groundedGenerate({ apiKeys, model, effort, systemPrompt, searchPrompt, buildStructurePrompt, schema, fallbackQueries }) {
 ...
 */
/**
 * 지난 조사에서 이미 정리해둔 내용을 프롬프트에 넣을 형태로 (lib/insights.js 의 기록).
 * 같은 불량을 다시 조사할 때 같은 답을 반복하지 않고 새로운 관점을 찾게 하는 것이 목적이다.
 * 프롬프트가 지나치게 길어지지 않게 항목 수와 길이를 제한한다.
 */
function knownBlock(known) {
  if (!known) return '';
  const lines = (list, title) => {
    const items = (list || []).slice(0, 12).map((x) => `- ${String(x.text || '').slice(0, 120)}`);
    return items.length ? `[${title}]\n${items.join('\n')}\n` : '';
  };
  const body = lines(known.causes, '이미 정리된 원인') + lines(known.actions, '이미 정리된 조치') + lines(known.measures, '이미 정리된 대책');
  if (!body) return '';
  return `\n===== 이전 조사에서 이미 확보한 내용 =====\n${body}=========================================\n`;
}
...
async function research(opt) {
  ...
  const known = knownBlock(opt.known);

  const searchPrompt = `자동차 부품 제조의 "${topic}"에 대해 웹에서 조사하라.

${opt.description ? `[불량 설명]\n${opt.description}\n` : ''}${opt.note ? `[현장 관찰]\n${opt.note}\n` : ''}${
    known ? `${known}위 내용은 이미 알고 있다. 같은 내용을 다시 찾지 말고, 여기에 없는 새로운 사례·원인·대책을 우선해서 조사하라.\n` : ''
  }
다음을 조사해 정리하라:
...
${
  known
    ? `${known}위 내용은 이미 확보되어 화면에 함께 표시된다. 같은 말을 다시 쓰지 말고, 여기에 없는 관점을 우선해서 채워라.
다만 개수는 반드시 채워야 하므로, 새로운 관점이 부족하면 기존 내용보다 더 구체적인(수치·조건이 담긴) 형태로 작성하라.\n`
    : ''
}
