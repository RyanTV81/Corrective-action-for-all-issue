...
const DEFAULTS = {
  apiKey: '',
  apiKey2: '', // 선택: 두 번째 무료 키 — 등록하면 요청을 두 키에 번갈아 분산해 무료 한도(RPM)를 사실상 2배로 늘린다
...
function get() {
  ...
  cache._key2FromEnv = false;
  const envKey2 = process.env.GEMINI_API_KEY_2 || process.env.GOOGLE_API_KEY_2;
  if (!cache.apiKey2 && envKey2) {
    cache.apiKey2 = envKey2.trim();
    cache._key2FromEnv = true;
  }
...
function save(patch, opt = {}) {
  ...
  if (typeof patch.apiKey2 === 'string') {
    const v = patch.apiKey2.trim();
    if (!v.includes('•')) next.apiKey2 = v; // 마스킹된 값이 되돌아온 경우 무시
  }
  ...
  if (cur._key2FromEnv && next.apiKey2 === cur.apiKey2) toWrite.apiKey2 = '';
...
function status() {
  ...
  return {
    ...
    hasApiKey2: Boolean(c.apiKey2),
    apiKey2Masked: c.apiKey2 ? `${c.apiKey2.slice(0, 8)}${'•'.repeat(10)}${c.apiKey2.slice(-4)}` : '',
    key2FromEnv: Boolean(c._key2FromEnv),
    model: c.model,
    effort: c.effort,
    useAI: c.useAI,
    useWeb: c.useWeb,
