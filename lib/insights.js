...
/**
 * 이만큼 쌓였으면 "이미 아는 불량"으로 보고 인터넷 조사를 다시 하지 않는다.
 * 조사 1회가 원인·조치·대책 10개씩을 채우므로, 정상적으로 한 번 조사한 불량은 여기에 걸린다
 * (중복 제거로 몇 개 빠질 수 있어 10이 아니라 8로 잡았다).
 * 다시 조사하게 하려면 관리자가 [쌓인 조사]에서 그 불량의 내용을 지우면 된다.
 */
const ENOUGH = 8;
...
/**
 * 이미 충분히 아는 불량인가 — 참이면 인터넷 조사(API 호출)를 건너뛰고 쌓인 내용으로 답한다.
 * 매번 다시 물어보면 API 사용량만 쌓이고, 이미 정리해둔 것과 비슷한 답이 돌아온다.
 */
function isEnough(rec) {
  if (!rec) return false;
  return ['causes', 'actions', 'measures'].every((k) => (rec[k] || []).length >= ENOUGH);
}
...
module.exports = { remember, find, isEnough, stats, list: listAll, remove, removeItem, ENOUGH };
