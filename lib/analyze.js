...
async function analyze(opt) {
  ...
  const known = defectLabel ? insights.find(memoryKey) : null;
  // 이미 충분히 아는 불량이면 인터넷을 다시 뒤지지 않는다 — 쌓아둔 내용으로 답한다(API 사용량 절약).
  // 다시 조사하게 하려면 [사용자 관리 → 쌓인 조사]에서 그 불량의 내용을 지우면 된다.
  const alreadyKnown = insights.isEnough(known);
    ...
    if (alreadyKnown) {
      ...
      try {
        web = await ai.research({
          ...
          // 일부만 쌓여 있는 경우 — 같은 답을 반복하지 말고 없는 것을 채우게 한다
          known,
          lang: opt.lang
        });
  ...
  const memory = web ? insights.find(memoryKey) : known;
  ...
  return {
    ...
    knowledge: memory
      ? {
          ...
          // 이미 알고 있어서 인터넷 조사를 건너뛴 경우 (사용자가 조사를 끈 것과 구분해서 안내한다)
          servedWithoutSearch: searchSkipped
        }
      : null,
    counts: {
