  ...
  if (fs.existsSync(DEFECT_DIR)) {
    for (const f of fs.readdirSync(DEFECT_DIR).filter((n) => n.endsWith('.json')).sort()) {
      try {
        ...
        // 관리자가 [KB 등록]으로 직접 넣은 불량은 표시해 둔다 (화면에서 출처를 구분해 보여준다)
        const learned = d.source === 'learned';
        if (Array.isArray(d.defects)) defects.push(...(learned ? d.defects.map((x) => ({ ...x, learned: true })) : d.defects));
      } catch (e) {
...
function matchDefects(opt) {
  ...
  const proc = opt.processId ? getProcess(opt.processId) : null;
  const pid = proc ? proc.id : null;

  const pool = proc ? proc.defects : kb.defects;
  const results = [];
...
function getProcess(id) {
  const kb = load();
  // 사용자가 보낸 값이 그대로 들어오므로 실제로 등록된 공정인지 확인한다
  // (__proto__ / constructor 같은 특수 키로 엉뚱한 객체가 잡히는 것을 막는다)
  if (typeof id !== 'string' || !Object.prototype.hasOwnProperty.call(kb.processMap, id)) return null;
  return kb.processMap[id];
}
...
function visualCueList(processId) {
  ...
  const proc = processId ? getProcess(processId) : null;
  if (proc) {
    (proc.visualCues || []).forEach(add);
    proc.defects.forEach((d) => (d.visualCues || []).forEach(add));
  } else {
