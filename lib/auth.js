...
function login(req, res) {
  ...
  if (username === ADMIN_USER && safeEqual(password || '', ADMIN_PASSWORD)) {
    ...
    activity.log({ username: ADMIN_USER, role: 'admin', action: 'login', label: '로그인', detail: loginDetail(req), ip: req.ip });
  ...
  activity.log({ username: u.username, role: 'user', action: 'login', label: '로그인', detail: loginDetail(req), ip: req.ip });
